...
import com.kalpanaaafinance.modules.shared.service.ProfileService;
...
public class InvestmentService {
    ...
    public InvestmentDTO confirmInvestment(InvestmentConfirmRequest request, User user) {
        ...
        // Step 1: Acquire FOR UPDATE lock on user's wallet Account, create if missing
        ...
        } else if (user.getBalance() != null && user.getBalance().compareTo(account.getBalance()) > 0) {
        ...
        if (balanceBefore.compareTo(request.getInvestedAmount()) < 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Insufficient wallet balance. Available: ₹" + balanceBefore);
        }
