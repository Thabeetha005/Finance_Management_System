...
const AuditLogs = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['auditlogs', pageIndex, search],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      const res = await axios.get(`http://localhost:5000/api/${'auditlogs'}?page=${pageIndex}&search=${search}`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/${'auditlogs'}/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    },
    onSuccess: () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto w-full animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <h1 className="text-3xl font-bold text-gray-900">AuditLogs</h1>
