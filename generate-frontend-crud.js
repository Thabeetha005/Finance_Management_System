...
const entities = ['Transactions', 'Invoices', 'Income', 'Expenses', 'Clients', 'Vendors', 'Accounts', 'Budgets', 'Notifications', 'Users', 'AuditLogs', 'Settings'];

entities.forEach(page => {
  const entityLower = page.toLowerCase();
...
const ${page} = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['${entityLower}', pageIndex, search],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      const res = await axios.get(\`http://localhost:5000/api/\${'${entityLower}'}?page=\${pageIndex}&search=\${search}\`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const token = localStorage.getItem('token');
      await axios.delete(\`http://localhost:5000/api/\${'${entityLower}'}/\${id}\`, {
        headers: { Authorization: \`Bearer \${token}\` }
      });
    },
    onSuccess: () => {
...
console.log('Frontend full CRUD components generated.');
